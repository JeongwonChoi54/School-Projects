...
#############################################################################
class State:
    def __init__(self, grid, own_pieces_loc, path, pathcost, pathcost_grid):
        self.grid = grid
            ...
            for j in range(-1, 2):
                if i == 0 and j == 0:
                    ...
                    if (x >= 0 and x < rows) and (y >= 0 and y < cols):
                        if grid[x][y] > 0:
                            new_pathcost = self.pathcost + grid[x][y]
                            if self.pathcost_grid[x][y] == 0 or self.pathcost_grid[x][y] > new_pathcost:
                                self.pathcost_grid[x][y] = new_pathcost
                                new_state = StateComp(State(grid, (x, y), self.path + [[(chr(self.king_y + 97), self.king_x), (chr(y+97), x)]],
                                                new_pathcost, self.pathcost_grid))
...
class StateComp(State):
    def __init__(self, state):
        ...
        self.pathcost = state.pathcost
        self.pathcost_grid = state.pathcost_grid
        super().__init__(self.grid, (self.king_x, self.king_y), self.path, self.pathcost, self.pathcost_grid)
...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    frontier = State(grid, own_pieces[0][1], [], 0, pathcost_grid).move(rows, cols)

    while not frontier.empty():
        current = frontier.get()
        if finish[current.king_x][current.king_y] == 1:
            return current.path, current.pathcost
